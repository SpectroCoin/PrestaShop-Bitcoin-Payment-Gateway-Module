...
$t = new TestRunner();
echo "SpectroCoin PrestaShop — plugin identification header\n\n";

$t->run('the client declares its platform and version', function ($t) use ($source, $root) {
    $t->assertSame('PrestaShop', constant_in($source, 'PLUGIN_PLATFORM'), 'PLUGIN_PLATFORM');
    // Taken from composer.json rather than written out here, so a release does
    // not have to remember to edit this file - the point is that the two agree.
    $declared = json_decode(file_get_contents($root . 'composer.json'), true)['version'];
    $t->assertSame($declared, constant_in($source, 'PLUGIN_VERSION'), 'PLUGIN_VERSION');
});
...
$t->run('the advertised version matches the plugin version', function ($t) use ($root) {
    ...
    $t->assertTrue(
        ...
        'composer.json must declare a plain semver version, got ' . var_export($declared, true)
    );
});
